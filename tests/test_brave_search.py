...
async def test_brave_search_tool():
    ...
    try:
        ...
        try:
            ...
            print(f"   ✅ 空查询错误处理正确: {e!s}")

        try:
            await brave_tool.execute("   ")  # 空白查询
        except Exception as e:
            print(f"   ✅ 空白查询错误处理正确: {e!s}")

    except Exception as e:
        print(f"❌ 初始化 Brave Search 工具时发生错误: {e!s}")
        print(f"错误类型: {type(e).__name__}")
